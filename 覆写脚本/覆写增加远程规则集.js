function main(config) {
    ...
    if (config["proxy-groups"]?.[0]?.proxies) {
      config["proxy-groups"][0].proxies = [
        ...config["proxy-groups"][0].proxies.filter(p => p.includes("集群测试")),
        ...config["proxy-groups"][0].proxies.filter(p => !p.includes("集群测试"))
      ];
    }
    ...
    config["rule-providers"]["ru-ruleset"] = {
        type: "http",
        behavior: "classical",
        path: "./ruleset/ru.yaml",
        url: "https://raw.githubusercontent.com/q1343135559/url-rule/refs/heads/main/ru.yaml",
        interval: 86400
    };
    ...
    const rules = [
      ...
      // "DOMAIN-KEYWORD,ozon,集群测试-俄罗斯",
      "RULE-SET,ru-ruleset,集群测试-俄罗斯"
    ];
